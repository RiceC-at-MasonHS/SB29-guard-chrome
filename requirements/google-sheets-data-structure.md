...
Further, there is a file called `example.csv` that can serve as a copy/paste-able template for the simplest possible dataset. It is populated with sample data for consideration, but remove all but the headers if you intend to implement this somewhere.
...
| `hostname` | String | Recommended. The explicit domain to match against. Do not include https:// or paths. This overrides resource_link for matching. | `instructure.com` |
...
*   **ID column:** (Optional) - You can elect to add an `id` column, but these values MUST be unique identifiers. If ommitted, the extension makes practical stand-ins.
*   **Other Recommended Columns:** These columns will complete all the information that is used to populate the 'viewer' pages of the extension. These come from the table below.
    *    **`vendor_name`:** The official name of the software vendor.
    *    **`resource_category`:** Broader categorization of the resource (e.g., `LMS`, `Productivity Tool`, etc).
    *    **`privacy_policy_link`:** Direct link to the Privacy policy that is always followed by the vendor, ragardless of DPA status.


## 2. Recommended Internal DPA Management Columns (for District Use)

Districts are recommended to collect and manage additional data points in their master Google Sheet to support a comprehensive DPA review and management process. These columns are not directly consumed by the extension's published CSV but are crucial for internal record-keeping, compliance, and decision-making. We recommend you have *at least* these columns, in addition to those in the table above. 
...
| `vendor_contact_name`| String    | The name of the contact for the software vendor.                                    | `Jimmy Kimmel`                                 |
...
| `requester_name`     | String    | Name of the staff member who requested the DPA review.                              | `John Smith`                                   |
| `requester_email`    | String    | Email of the staff member who requested the DPA review.                             | `john.smith@schooldistrict.org`                |
...
| `privacy_policy_link`| URL       | Direct link to the application's general Privacy policy (ragardles of DPA)          | `https://github.com/privacy`                   |
...
| `use_instructions`   | String    | Detailed instructions for teachers regarding the use of the software.               | `Online collaboration and storage ...`         |
...
2.  **Your School Email Address:** (Short answer text)
    ...
    *   Recommended: automatically collect emails, so this question can be removed.
...
5.  **Hostname for the Software/Application:** (Short answer text)
    *   *Maps to:* `hostname`
    *   Make optional - this will be difficult for some to teachers to determine. Whoever manages the Google Sheet can adjust the hostname until it works properly with the extension. 
6.  **Official Company/Vendor Name:** (Short answer text)
    *   *Maps to:* `vendor_name`, `company_name`
7.  **Vendor's Legal/Privacy Contact Email (if known):** (Short answer text, validation: email)
    *   *Maps to:* `vendor_email`
8.  **Briefly describe the educational purpose of this software/application:** (Paragraph text)
    *   *Maps to:* `purpose`
9.  **In which grades/student levels will this software/application be used?** (Checkboxes: `K`, `1`, `2`, ..., `12`, `Adult Education`, `Other (please specify)`)
    *   *Maps to:* `grades_used`
10.  **In which school building(s) will this software/application be used?** (Checkboxes: List all district schools, `Other (please specify)`)
    *   *Maps to:* `building`
11. **Which content area(s) or department(s) will use this software/application?** (Checkboxes: `English Language Arts`, `Math`, `Science`, `Social Studies`, `Arts`, `Music`, `Physical Education`, `Health`, `World Languages`, `Special Education`, `Applied Technology`, `STEAM`, `Other (please specify)`)
    *   *Maps to:* `content_area`
12. **Are there specific courses where this software/application will be used?** (Paragraph text)
    *   *Maps to:* `courses`
13. **Estimated number of teachers who will use this software/application:** (Number)
    *   *Maps to:* `number_teachers`
14. **Estimated number of students who will use this software/application:** (Number)
    *   *Maps to:* `number_students`
15. **How does this software/application support student learning?** (Paragraph text)
    *   *Maps to:* `how_support_learning`
16. **How does this software/application address accessibility needs for students?** (Paragraph text)
    *   *Maps to:* `how_accessible_needs`
17. **Is there a cost associated with this software/application? If so, please describe the cost and licensing model.** (Paragraph text)
    *   *Maps to:* `cost`
18. **What type of resource is this?** (Checkboxes: `Instructional Tool`, `Assessment`, `Communication Platform`, `Content Library`, `Skill Practice`, `Staff/Teacher Resource`, `Other (please specify)`)
    *   *Maps to:* `resource_type`, `resource_other`
19. **Please provide any additional relevant information, notes, or instructions for using this software/application.** (Paragraph text)
    *   *Maps to:* `anything_else`, `use_instructions`
20. **Do students log in directly to this resource? If yes, please provide the student login URL (if different from the main website).** (Short answer text, validation: URL)
    *   *Maps to:* `student_login_link`
21. **Do staff/teachers log in directly to this resource? If yes, please provide the staff/teacher login URL (if different from the main website).** (Short answer text, validation: URL)
    *   *Maps to:* `staff_login_link`
22. **Have you already spoken to a sales representative from this vendor about educational pricing or terms?** (Multiple choice: `Yes`, `No`)
    *   *Maps to:* `spoken_to_sales`
23. **If available, please provide a direct link to the vendor's Data Protection Addendum (DPA) or Privacy Policy.** (Short answer text, validation: URL)
    *   *Maps to:* `dpa_link`
